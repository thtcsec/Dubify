...
# Start Backend
Start-Process -FilePath "powershell" -ArgumentList "-NoExit", "-Command", "Write-Host 'Starting Backend...' -ForegroundColor Green; cd backend; if (Test-Path '.\venv\Scripts\Activate.ps1') { & .\venv\Scripts\Activate.ps1; uvicorn app.main:app --reload } else { Write-Error 'Virtual environment not found! Run pip install -r requirements.txt first.' }"
